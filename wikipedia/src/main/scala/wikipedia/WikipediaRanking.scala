...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.
    filter(_.mentionsLanguage(lang)).aggregate(0)((result, _) => result + 1, _ + _)
//    (for {
//    article <- rdd
//    if article.text.split(" ").contains(lang)
//  } yield article).aggregate(0)((result, _) => result + 1, _ + _)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(_.size).sortBy({case (lang, count) => count}, false).collect().toList
...
}.reduceByKey((acc, i) => acc + i).sortBy({case (lang, count) => count}, false).collect().toList
