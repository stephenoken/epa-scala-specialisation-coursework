...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def contains(x: Tweet): Boolean =
    if (x.text < elem.text) left.contains(x) // if the text is lexicographically less check left
    else if (elem.text < x.text) right.contains(x) // if greater check right
    else true // if the same true
