...
//    (dictionary foldLeft Map.empty[Occurrences, List[Word]].withDefaultValue(List())) {
//      (result, word) => {
//        val key = wordOccurrences(word)
//        result + (key -> (word :: result(key)))
//      }
//    }
...
def occurrenceAnagrams(occurrences: Occurrences): List[Sentence] =
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = occurrenceAnagrams(sentenceOccurrences(sentence))
